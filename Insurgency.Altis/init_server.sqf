...
[] spawn {
	...
	if (!isNil "MHQ_3") then {
		...
		if (INS_MHQ_enabled) then {
			...
			_nul = [MHQ_3, 60, 0.01, {_this setVariable["persistent",true]; _VarName = "MHQ_3"; _veh setVehicleVarName _VarName; _veh Call Compile Format ["%1=_This ; PublicVariable ""%1""",_VarName]; INS_MHQ_killed = "MHQ_3"; publicVariable "INS_MHQ_killed";}] execVM "vehrespawn.sqf";
		};
	};
	if (!isNil "MHQ_4") then {
		MHQ_4 setVariable ["persistent",true];
		if (INS_MHQ_enabled) then {
			MHQ_4 addEventhandler ["killed","[(_this select 0)] call INS_MHQ_VarName"];
			_nul = [MHQ_4, 60, 0.01, {_this setVariable["persistent",true]; _VarName = "MHQ_4"; _veh setVehicleVarName _VarName; _veh Call Compile Format ["%1=_This ; PublicVariable ""%1""",_VarName]; [_this] call anti_collision; INS_MHQ_killed = "MHQ_4"; publicVariable "INS_MHQ_killed";}] execVM "vehrespawn.sqf";
		};
	};
	if (!isNil "Marine_MHQ") then {
		Marine_MHQ setVariable["persistent",true];
		if (INS_MHQ_enabled) then {
			Marine_MHQ addEventhandler ["Respawn","[(_this select 0)] call INS_MHQ_VarName"];
			_nul = [Marine_MHQ, 60, 0.01, {_this setVariable["persistent",true]; _VarName = "Marine_MHQ"; _veh setVehicleVarName _VarName; _veh Call Compile Format ["%1=_This ; PublicVariable ""%1""",_VarName]; INS_MHQ_killed = "Marine_MHQ"; publicVariable "INS_MHQ_killed";}] execVM "vehrespawn.sqf";[_this] call anti_collision;
		};
	};
};
...
if (DebugEnabled isEqualTo 1) then {
	...
	_null = [] execVM "eos1\OpenMe.sqf";																// EOS (urban mission and defend AO)
