...
[] spawn {
	...
	MHQ_4 addAction[("<t color='#F56618'>") + (localize "STR_BMR_load_VAprofile") + "</t>","=BTC=_revive\=BTC=_addAction.sqf",[[],JIG_load_VA_profile_MHQ3], 1, true, true, "", "side _this != EAST"];
		...
		If (side player == east) then {
			...
			[] spawn {
				...
				if (INS_MHQ_enabled) then {
					...
					_op4 = true;
					_mhqObj = objNull;
					_mhqObj = ["Marine_MHQ"] call mhq_obj_fnc;
				...
				if (!isNil "MHQ_4") then {removeAllActions MHQ_4;};
			};
		};
		If (side player == west) then {
			...
			[] spawn {
				...
				if (INS_MHQ_enabled) then {
					...
					_mhqObj4 = ["MHQ_3"] call mhq_obj_fnc;
					_mhqPos = getPos MHQ_4; _nul = [_mhqObj4,_op4,_mhqPos] spawn INS_MHQ_mkr;
					...
					INS_flag addAction["<t size='1.5' shadow='2' color='#ED2744'>Transfer to MHQ_4</t>", "call JIG_transfer_fnc", ["MHQ_4"], 4];
				};
			};
		};
	}
	else
	{
		...
		If (side player == west) then {
			...
			if (INS_MHQ_enabled) then {
				...
				_mhqObj3 = ["MHQ_3"] call mhq_obj_fnc;
				_mhqPos = getPos MHQ_3; _nul = [_mhqObj3,_op4,_mhqPos] spawn INS_MHQ_mkr;
				_mhqObj4 = ["MHQ_4"] call mhq_obj_fnc;
				_mhqPos = getPos MHQ_4; _nul = [_mhqObj4,_op4,_mhqPos] spawn INS_MHQ_mkr;
				INS_flag addAction["<t size='1.5' shadow='2' color='#ED2744'>Transfer to MHQ_1</t>", "call JIG_transfer_fnc", ["MHQ_1"], 4.2];
				INS_flag addAction["<t size='1.5' shadow='2' color='#ED2744'>Transfer to MHQ_2</t>", "call JIG_transfer_fnc", ["MHQ_2"], 4.1];
				INS_flag addAction["<t size='1.5' shadow='2' color='#ED2744'>Transfer to MHQ_3</t>", "call JIG_transfer_fnc", ["MHQ_3"], 4];
