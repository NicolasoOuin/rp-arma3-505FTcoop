...
vts_getisGM=
{
  ...
  if !(hasinterface) exitwith {false;};
	/* ---------------
	_namel = _this name;
	player globalchat _namel;

	if (MF_ISGM==1) exitwith {true;}; 
	----------------- */
  //Everybodys GM?
...
vts_isNotmove={
 ...
 if (true) then { _object forceSpeed 0;};
  if ([player] call vts_getisGM) then {"IA not move active" call vts_gmmessage;};
};
if (isserver) then {publicvariable "vts_isNotmove";};
...
vts_objetfixe={
 ...
 if (true) then { _object setPos [getPos _object select 0, getPos _object select 1, getPos _object select 2];_object enableSimulation false;};
 
  if ([player] call vts_getisGM) then {"Objet fixe" call vts_gmmessage;};
};
if (isserver) then {publicvariable "vts_isNotmove";};
