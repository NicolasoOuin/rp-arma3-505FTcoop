				...
				_units=_eosActivated getvariable _cacheGrp;
						...
						_aMin=_aSize select 0;
							if (_debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
								...
								if (_aMin > 0) then {
										_aGroup=[_mPos,_aSize,_faction,_side] call EOS1_fnc_spawngroup;
										if (!surfaceiswater _mPos) then {
											0=[_mPos,units _aGroup,_mkrX,0,[0,20],true,true] call shk1_fnc_fillhouse;
												}else{
												0 = [_aGroup,_mkr] call EOS1_fnc_taskpatrol;
													};
												_aGrp set [count _aGrp,_aGroup];
												0=[_aGroup,"INFskill"] call eos1_fnc_grouphandlers;
if (_debug) then {PLAYER SIDECHAT (format ["Spawned House Patrol: %1",_counter]);0= [_mkr,_counter,"House Patrol",getpos (leader _aGroup)] call EOS1_debug};
												};
		};
		...
		if (_cache) then {
				...
				_units=_eosActivated getvariable _cacheGrp;
					...
					if (_debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
							...
							if (_bMin > 0) then {
									_pos = [_mkr,true] call SHK1_pos;
									_bGroup=[_pos,_bSize,_faction,_side] call EOS1_fnc_spawngroup;
										0 = [_bGroup,_mkr] call EOS1_fnc_taskpatrol;
										_bGrp set [count _bGrp,_bGroup];

										0=[_bGroup,"INFskill"] call eos1_fnc_grouphandlers;
if (_debug) then {PLAYER SIDECHAT (format ["Spawned Patrol: %1",_counter]);0= [_mkr,_counter,"patrol",getpos (leader _bGroup)] call EOS1_debug};
												};
		};
...
//SPAWN LIGHT VEHICLES
	...
	if (isnil "_cGrp") then {_cGrp=[];};
		...
		_newpos=[_mkr,50] call EOS1_fnc_findSafePos;
			if (surfaceiswater _newpos) then {_vehType=8;_cargoType=10;}else{_vehType=7;_cargoType=9;};
				...
				if ((_cSize select 0) > 0) then{
						0=[(_cGroup select 0),_cSize,(_cGroup select 2),_faction,_cargoType] call eos1_fnc_setcargo;
						};

							0=[(_cGroup select 2),"LIGskill"] call eos1_fnc_grouphandlers;
								0 = [(_cGroup select 2),_mkr] call EOS1_fnc_taskpatrol;
								_cGrp set [count _cGrp,_cGroup];

if (_debug) then {player sidechat format ["Light Vehicle:%1 - r%2",_counter,_cGrps];0= [_mkr,_counter,"Light Veh",(getpos leader (_cGroup select 2))] call EOS1_debug};
		};
	...
	if (isnil "_dGrp") then {_dGrp=[];};
		...
		_newpos=[_mkr,50] call EOS1_fnc_findSafePos;
			if (surfaceiswater _newpos) then {_vehType=8;}else{_vehType=2;};
					...
					_dGroup=[_newpos,_side,_faction,_vehType]call EOS1_fnc_spawnvehicle;

						0=[(_dGroup select 2),"ARMskill"] call eos1_fnc_grouphandlers;
						0 = [(_dGroup select 2),_mkr] call EOS1_fnc_taskpatrol;
							_dGrp set [count _dGrp,_dGroup];

if (_debug) then {player sidechat format ["Armoured:%1 - r%2",_counter,_dGrps];0= [_mkr,_counter,"Armour",(getpos leader (_dGroup select 2))] call EOS1_debug};
		};
...
//SPAWN STATIC PLACEMENTS
	for "_counter" from 1 to _eGrps do {
		...
		_newpos=[_mkr,50] call EOS1_fnc_findSafePos;
					...
					_eGroup=[_newpos,_side,_faction,5]call EOS1_fnc_spawnvehicle;
1
						0=[(_eGroup select 2),"STAskill"] call eos1_fnc_grouphandlers;
							_eGrp set [count _eGrp,_eGroup];

if (_debug) then {player sidechat format ["Static:%1",_counter];0= [_mkr,_counter,"Static",(getpos leader (_eGroup select 2))] call EOS1_debug};
		};
		...
		_fGroup set [count _fGroup,_cargoGrp];
			null = [_mkr,_fGroup,_counter] execvm "eos1\functions1\TransportUnload1_fnc.sqf";
					...
					_wp1 setWaypointType "SAD";};
						...
						0=[(_fGroup select 2),"AIRskill"] call eos1_fnc_grouphandlers;
...
if (!(getmarkercolor _mkr == "colorblack")) then {
	null = [_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_eGrps,_fGrps,_fSize],_settings,true] execVM "eos1\core1\eos1_core.sqf";
