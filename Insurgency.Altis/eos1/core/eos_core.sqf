...
if ((_fSize select 0) > 0) then {
	_cargoGrp = createGroup _side;
		...
		_fGroup set [count _fGroup,_cargoGrp];
			null = [_mkr,_fGroup,_counter] execvm "eos1\functions\TransportUnload_fnc.sqf";
				}else{
...
if (!(getmarkercolor _mkr == "colorblack")) then {
	null = [_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_eGrps,_fGrps,_fSize],_settings,true] execVM "eos1\core\eos_core.sqf";
